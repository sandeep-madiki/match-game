...
import ThumbnailItems from '../thumbnails'
...
class GameMain extends Component {
  ...
  getFilteredItems = () => {
    const {itemsList, activeTabId} = this.state
    const filteredProjects = itemsList.filter(
  ...
  clickedThumbnailId = id => {
    const {displayImg} = this.state
    if (id === displayImg.id) {
      const randNum = Math.ceil(Math.random() * 29)
      const imgUrl = imagesList[randNum]
      this.setState(prev => ({
        score: prev.score + 1,
        displayImg: imgUrl,
      }))
  ...
  runningGame = () => {
    const filteredRes = this.getFilteredItems()
    const {displayImg, activeTabId} = this.state
    return (
      <>
        <img src={displayImg.imageUrl} alt="match" className="display-img" />
        <ul className="tabs-container">
          {tabsList.map(each => (
            <TabItems
              details={each}
              key={each.tabId}
              displayFunc={this.changeCategory}
              isActive={activeTabId === each.tabId}
            />
          ))}
        </ul>
        <ul className="thumbnails-container">
          {filteredRes.map(each => (
            <ThumbnailItems
              details={each}
              key={each.id}
              getThumbnailId={this.clickedThumbnailId}
            />
          ))}
        </ul>
      </>
  ...
  scoreCardDetails = () => {
    ...
    return <ScoreCard score={score} resetGame={this.playAgainGame} />
  }
  ...
  render() {
    ...
    const runGame = this.runningGame()
    const scoreDetails = this.scoreCardDetails()
    const renderGame = time > 0 ? runGame : scoreDetails
    return (
      <div className="main-container">
        <ul className="nav_list">
          <Navbar score={score} time={time} clearTimer={this.clearInterval0} />
        </ul>
        <div className="bg-img">{renderGame}</div>
      </div>
    )
  }
}
